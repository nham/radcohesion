...
Interceptor: {"": "Object;",
  ...
  "%": "DOMError|FileError|MediaError|MediaKeyError|Navigator|NavigatorUserMediaError|PositionError|SQLError|SVGAnimatedLength|SVGAnimatedNumberList|SVGRect|WebGLBuffer|WebGLProgram|WebGLShader"
},
...
main: function() {
  var t1, canvas, e, t2, exception, p;
  t1 = {};
  ...
  t1.gl_0 = null;
  try {
    t1.gl_0 = E.glContextSetup(canvas);
  } catch (exception) {
    t1 = H.unwrapException(exception);
    e = t1;
    P.print(e);
...
glContextSetup: function(canvas) {
  ...
  J.clearColor$4$x(gl, 0.5, 0.5, 0.5, 1);
  gl.clearDepth(1);
...
gridBufferSetup: function(gl) {
  var pbuf, ibuf, cbuf, sl, t1, a, purp, blue, green, orange, salmon, colors;
  pbuf = gl.createBuffer();
  ibuf = gl.createBuffer();
  cbuf = gl.createBuffer();
  $.triGrid = new E.Figure(pbuf, ibuf, cbuf, [0, 1.8, -9], 0);
  sl = Math.tan(1.0471975511965976);
  t1 = Math.cos(1.0471975511965976);
  a = E.gridifyTriangle([-1.6, -1.6 / sl, 0], [t1, Math.sin(1.0471975511965976), 0], [1, 0, 0], 3.2);
...
gridifyTriangle: function(startingPoint, leftEdge, botEdge, s) {
  var t1, t2, a, i, x, t3, j, y, b, i0, c, off, z;
  t1 = new E.gridifyTriangle_scaleV();
  t2 = new E.gridifyTriangle_addV();
  a = P.List_List(5, null);
  for (i = 0; i < 5; ++i) {
    x = t2.call$2(t1.call$2(leftEdge, i * s / 4), startingPoint);
    a[i] = P.List_List(null, null);
    a[i].push(x);
    for (t3 = 5 - i, j = 1; j < t3; ++j) {
      y = t1.call$2(botEdge, j * s / 4);
  ...
  t1 = new E.gridifyTriangle_addVtoc(c);
...
GlProgram: {"": "Object;attributes,uniforms,program,fragShader,vertShader,gl",
  GlProgram$5: function(gl, fragSrc, vertSrc, attributeNames, uniformNames) {
    ...
    for (t1 = new H.ListIterator(attributeNames, 2, 0, null), t2 = this.attributes; t1.moveNext$0();) {
      attrib = t1._current;
...
main_animate: {"": "Closure;box_0",
  call$1: function(now) {
    var t1, elapsed, t2, t3;
    t1 = this.box_0;
    if (!J.$eq(t1.lastTime_1, 0)) {
      elapsed = J.$sub$n(now, t1.lastTime_1);
      t2 = $.triGrid;
      t3 = t2.ang;
...
gridifyTriangle_scaleV: {"": "Closure;",
  call$2: function(xs, c) {
    var t1, t2;
    t1 = J.getInterceptor$asx(xs);
    t2 = J.getInterceptor$n(c);
...
gridifyTriangle_addVtoc: {"": "Closure;c_0",
  call$1: function(v) {
    var t1, t2;
    t1 = this.c_0;
    t2 = J.getInterceptor$asx(v);
...
Figure: {"": "Object;posBuf,indexBuf,colorBuf,pos,ang"}},
1],
]);
...
function dart_precompiled($collectedClasses) {
  ...
  function main_tick(box_0, canvas_1, p_2, animate_3) {
    this.box_0 = box_0;
    this.canvas_1 = canvas_1;
    this.p_2 = p_2;
    this.animate_3 = animate_3;
  }
  main_tick.builtin$cls = "main_tick";
  if (!"name" in main_tick)
    main_tick.name = "main_tick";
  $desc = $collectedClasses.main_tick;
  if ($desc instanceof Array)
    $desc = $desc[1];
  main_tick.prototype = $desc;
  function gridBufferSetup_scaleV() {
  }
  gridBufferSetup_scaleV.builtin$cls = "gridBufferSetup_scaleV";
  if (!"name" in gridBufferSetup_scaleV)
    gridBufferSetup_scaleV.name = "gridBufferSetup_scaleV";
  $desc = $collectedClasses.gridBufferSetup_scaleV;
  if ($desc instanceof Array)
    $desc = $desc[1];
  gridBufferSetup_scaleV.prototype = $desc;
  function gridBufferSetup_addColor(colors_0) {
    this.colors_0 = colors_0;
  }
  gridBufferSetup_addColor.builtin$cls = "gridBufferSetup_addColor";
  if (!"name" in gridBufferSetup_addColor)
    gridBufferSetup_addColor.name = "gridBufferSetup_addColor";
  $desc = $collectedClasses.gridBufferSetup_addColor;
  if ($desc instanceof Array)
    $desc = $desc[1];
  gridBufferSetup_addColor.prototype = $desc;
  function gridBufferSetup_add3(addColor_1) {
    this.addColor_1 = addColor_1;
  }
  gridBufferSetup_add3.builtin$cls = "gridBufferSetup_add3";
  if (!"name" in gridBufferSetup_add3)
    gridBufferSetup_add3.name = "gridBufferSetup_add3";
  $desc = $collectedClasses.gridBufferSetup_add3;
  if ($desc instanceof Array)
    $desc = $desc[1];
  gridBufferSetup_add3.prototype = $desc;
  function gridifyTriangle_scaleV() {
  }
  gridifyTriangle_scaleV.builtin$cls = "gridifyTriangle_scaleV";
  if (!"name" in gridifyTriangle_scaleV)
    gridifyTriangle_scaleV.name = "gridifyTriangle_scaleV";
  $desc = $collectedClasses.gridifyTriangle_scaleV;
  if ($desc instanceof Array)
    $desc = $desc[1];
  gridifyTriangle_scaleV.prototype = $desc;
  function gridifyTriangle_addV() {
  }
  gridifyTriangle_addV.builtin$cls = "gridifyTriangle_addV";
  if (!"name" in gridifyTriangle_addV)
    gridifyTriangle_addV.name = "gridifyTriangle_addV";
  $desc = $collectedClasses.gridifyTriangle_addV;
  if ($desc instanceof Array)
    $desc = $desc[1];
  gridifyTriangle_addV.prototype = $desc;
  function gridifyTriangle_addVtoc(c_0) {
    this.c_0 = c_0;
  }
  gridifyTriangle_addVtoc.builtin$cls = "gridifyTriangle_addVtoc";
  if (!"name" in gridifyTriangle_addVtoc)
    gridifyTriangle_addVtoc.name = "gridifyTriangle_addVtoc";
  $desc = $collectedClasses.gridifyTriangle_addVtoc;
  if ($desc instanceof Array)
    $desc = $desc[1];
  gridifyTriangle_addVtoc.prototype = $desc;
  function icosaBufferSetup_scaleV() {
  }
  icosaBufferSetup_scaleV.builtin$cls = "icosaBufferSetup_scaleV";
  if (!"name" in icosaBufferSetup_scaleV)
    icosaBufferSetup_scaleV.name = "icosaBufferSetup_scaleV";
  $desc = $collectedClasses.icosaBufferSetup_scaleV;
  if ($desc instanceof Array)
    $desc = $desc[1];
  icosaBufferSetup_scaleV.prototype = $desc;
  function icosaBufferSetup_addV() {
  }
  icosaBufferSetup_addV.builtin$cls = "icosaBufferSetup_addV";
  if (!"name" in icosaBufferSetup_addV)
    icosaBufferSetup_addV.name = "icosaBufferSetup_addV";
  $desc = $collectedClasses.icosaBufferSetup_addV;
  if ($desc instanceof Array)
    $desc = $desc[1];
  icosaBufferSetup_addV.prototype = $desc;
  function icosaBufferSetup_addVtoa(a_0) {
    this.a_0 = a_0;
  }
  icosaBufferSetup_addVtoa.builtin$cls = "icosaBufferSetup_addVtoa";
  if (!"name" in icosaBufferSetup_addVtoa)
    icosaBufferSetup_addVtoa.name = "icosaBufferSetup_addVtoa";
  $desc = $collectedClasses.icosaBufferSetup_addVtoa;
  if ($desc instanceof Array)
    $desc = $desc[1];
  ...
  if (!"name" in icosaBufferSetup_closure)
    icosaBufferSetup_closure.name = "icosaBufferSetup_closure";
  $desc = $collectedClasses.icosaBufferSetup_closure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  ...
  Figure.builtin$cls = "Figure";
  if (!"name" in Figure)
    Figure.name = "Figure";
  $desc = $collectedClasses.Figure;
  if ($desc instanceof Array)
    $desc = $desc[1];
  Figure.prototype = $desc;
